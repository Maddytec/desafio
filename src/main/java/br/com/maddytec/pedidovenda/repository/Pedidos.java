...
public class Pedidos implements Serializable {
	...
	private Criteria criarCriteriaParaFiltro(PedidoFilter filtro) {
		...
		if (filtro.getDataCriacaoDe() != null) {
			...
			calendar.setTime(filtro.getDataCriacaoDe());
			calendar.set(Calendar.MILLISECOND, 00);
			calendar.set(Calendar.SECOND, 00);
			calendar.set(Calendar.MINUTE, 00);
			calendar.set(Calendar.HOUR, 00);
			
			criteria.add(Restrictions.ge("dataCriacao", calendar.getTime()));
		}
		...
		if (filtro.getDataEntregaDe() != null) {
			Calendar calendar = Calendar.getInstance();
			calendar.setTime(filtro.getDataEntregaDe());
			calendar.set(Calendar.MILLISECOND, 00);
			calendar.set(Calendar.SECOND, 00);
			calendar.set(Calendar.MINUTE, 00);
			calendar.set(Calendar.HOUR, 00);
			
			criteria.add(Restrictions.ge("dataEntrega", calendar.getTime()));
		}

		if (filtro.getDataEntregaAte() != null) {
			Calendar calendar = Calendar.getInstance();
			calendar.setTime(filtro.getDataEntregaAte());
			calendar.set(Calendar.MILLISECOND, 59);
			calendar.set(Calendar.SECOND, 59);
			calendar.set(Calendar.MINUTE, 59);
			calendar.set(Calendar.HOUR, 23);

			criteria.add(Restrictions.le("dataEntrega", calendar.getTime()));
		}
